...
class Account extends MY_Controller{
  ...
  function index()
  {
    ...
    $this->template->view("content/account/index",$data);
  }
  //
  // function ubah_data()
  // {
  //   $this->template->set_title("Ubah Data");
  //   $data["row"] = $this->model->get_where_person();
  //   $data['action'] = site_url("backend/account/action_ubah_data");
  //   $this->template->view("content/account/form",$data);
  // }
  //
  //
  // function action_ubah_data()
  // {
  ...
  //           $json['success'] =  true;
  //           $json['url']  = site_url("backend/account");
  //         }else {
  ...
  function reset_password()
          {
            if ($this->input->is_ajax_request()) {
                if ($row =  $this->model->get_where("auth_person",["id_person"=>sess("id_person")])) {
                  $data['action'] = site_url("backend/account/reset_password_action");
                  $this->template->view("content/account/reset_pwd",$data,false);
                }else {
                  echo "page not found";
                }
            }
          ...
          // function berkas()
          // {
          //   $this->template->set_title("Ubah Berkas");
          //   $data["row"] = $this->model->get_where_person();
          //   $data['action'] = site_url("backend/account/is_complate");
          //   $this->template->view("content/account/berkas",$data);
          // }
          //
          //
          //
          ...
          function reset_password_action($id="")
          {
            if ($this->input->is_ajax_request()) {
              ...
              if ($this->form_validation->run()) {
                ...
                $this->model->get_update("auth_person",$data,["id_person"=>sess("id_person")]);
                $json['success'] = true;
